...
function ejsRef({ root, source, src, dist, projName }) {
  return function(obj, type, srcStack = []) {
    ...
    const { refPath, refProp } = toRealPath(ref, extname);

    // build real path
    srcStack = srcStack.map(function(path, index) {
      return index === 0 ? path: toRealPath(path).refPath;
    });
    ...
    try {
      ...
      if (srcStack.indexOf(refPath) === -1) {
        if (~['', 'oas3', 'yml'].indexOf(extname)) {
          ...
          if (refProp) {
            json = lodash.get(json, refProp);
          }
        } else if (extname === 'json') {
          json = JSON.parse(fs.readFileSync(refPath, 'utf8'));
        }
      }
      // if type is a schema $ref and not prop
      if (type === 'schema' && !refProp) {
    ...
    function toRealPath(ref, extname) {
      ...
      var rPath = /^#\/(components\/(?:schemas|examples|requestBodies|responses)\/[^/]+)(.*)/;
      const results = rPath.exec(ref);
      const path = pathFn.resolve(source, src, projName, results[1] + (!extname ? '.oas3' : ''));
      const prop = results[2].slice(1).replace('/', '.');
